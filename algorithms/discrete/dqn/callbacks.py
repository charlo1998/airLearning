...
import os, sys
# os.sys.path.insert(0,"c:\\workspace\\airlearning-rl\\")
import settings
import numpy as np
import msgs
...
class FileLogger(Callback):
	...
	def on_episode_end(self, episode, logs):
		...
		data += [('episode', episode), ('duration', duration), ('Final_Y', self.infos[0][0]),
		         ('Final_X', self.infos[0][1])]
...
class CheckPointLogger(Callback):
	...
	"""
		if (self.ctr % 20 == 0):
			#if (logs['mission_status'] == 'Success'):
			file_path_list = self.find_list_of_files(self.data_dir)
			new_file_path = self.make_up_file_name(file_path_list, self.data_dir)
			self.write(new_file_path)
		self.ctr +=1 
		"""
